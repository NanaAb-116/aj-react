import React, { useEffect } from 'react';

function News() {
  useEffect(() => {
    document.title = 'News | Nana A.J';
  }, []);
  return (
    <div>
      <section className='news_section'>
        <div className='container-fluid news'>
          <div className='row'>
            <div className='col-md-6 mx-auto tour_sec'>
              <h1 className='text-left text-light'>News</h1>
            </div>
          </div>
        </div>
      </section>
      <section className='tours'>
        <div className='container'>
          <div className='row my-5'>
            <div className='col-md-4 my-auto'>
              <h3>Nana A.J on Gorillas</h3>
              <a
                target='_blank'
                className='read-more'
                href='https://gorillas.io/en/blog/black-history-month-at-gorillas'
              >
                Ream More
              </a>
            </div>
            <div className='col-md-6 mx-4'>
              <a
                target='_blank'
                href='https://gorillas.io/en/blog/black-history-month-at-gorillas'
              >
                <img
                  className='img-fluid my-5'
                  src='/images/black-history.PNG'
                  alt=''
                />
              </a>
            </div>
          </div>
          <div className='row my-5'>
            <div className='col-md-4 my-auto small'>
              <h3>Nana A.J reassures on new single Bigger</h3>
              <a
                target='_blank'
                className='read-more'
                href='https://gorillas.io/en/blog/pedal-records-newest-artists'
              >
                Ream More
              </a>
            </div>
            <div className='col-md-6 mx-4'>
              <a
                target='_blank'
                href='https://gorillas.io/en/blog/pedal-records-newest-artists'
              >
                <img
                  className='img-fluid my-5'
                  src='/images/gor art.PNG'
                  width='500'
                  alt=''
                />
              </a>
            </div>
            <div className='col-md-4 my-auto big'>
              <h3>Nana A.J reassures on new single Bigger</h3>
              <a
                target='_blank'
                className='read-more'
                href='https://www.ghanaweb.com/GhanaHomePage/entertainment/Nana-A-J-reassures-on-new-single-Bigger-1280410'
              >
                Ream More
              </a>
            </div>
          </div>
          <div className='row my-5'>
            <div className='col-md-4 my-auto'>
              <h3>Nana A.J featured in Pedal Records launch</h3>
              <a
                target='_blank'
                className='read-more'
                href='https://gorillas.io/en/blog/gorillas-launches-pedal-records-music-label'
              >
                Ream More
              </a>
            </div>
            <div className='col-md-6 mx-4'>
              <a
                target='_blank'
                href='https://gorillas.io/en/blog/gorillas-launches-pedal-records-music-label'
              >
                <img className='img-fluid my-5' src='/images/gor.PNG' alt='' />
              </a>
            </div>
          </div>
          <div className='row my-5'>
            <div className='col-md-4 my-auto small'>
              <h3>Nana A.J reassures on new single Bigger</h3>
              <a
                target='_blank'
                className='read-more'
                href='https://www.ghanaweb.com/GhanaHomePage/entertainment/Nana-A-J-reassures-on-new-single-Bigger-1280410'
              >
                Ream More
              </a>
            </div>
            <div className='col-md-6 mx-4'>
              <a
                target='_blank'
                href='https://www.ghanaweb.com/GhanaHomePage/entertainment/Nana-A-J-reassures-on-new-single-Bigger-1280410'
              >
                <img
                  className='img-fluid my-5'
                  src='https://cdn.ghanaweb.com/imagelib/pics/104/10416965.295.jpg'
                  width='500'
                  alt=''
                />
              </a>
            </div>
            <div className='col-md-4 my-auto big'>
              <h3>Nana A.J reassures on new single Bigger</h3>
              <a
                target='_blank'
                className='read-more'
                href='https://www.ghanaweb.com/GhanaHomePage/entertainment/Nana-A-J-reassures-on-new-single-Bigger-1280410'
              >
                Ream More
              </a>
            </div>
          </div>
          <div className='row my-5'>
            <div className='col-md-4 my-auto'>
              <h3>
                Nana A.J's “Train of Thought” EP Is A Fleeting Express Worth
                Boarding
              </h3>
              <a
                target='_blank'
                className='read-more'
                href='https://kuulpeeps.com/2021/08/13/nana-a-js-train-of-thought-ep-is-a-fleeting-express-worth-boarding/pop-culture'
              >
                Ream More
              </a>
            </div>
            <div className='col-md-6 mx-4'>
              <a
                target='_blank'
                href='https://kuulpeeps.com/2021/08/13/nana-a-js-train-of-thought-ep-is-a-fleeting-express-worth-boarding/pop-culture'
              >
                <img
                  className='img-fluid my-5'
                  src='https://kuulpeeps.com/wp-content/uploads/2021/08/IMG_314.jpg'
                  height='500'
                  alt=''
                />
              </a>
            </div>
          </div>
          <div className='row my-5'>
            <div className='col-md-4 my-auto small'>
              <h3>
                Nana A.J's "Train of Thought" EP is a fleeting express worth
                boarding
              </h3>
              <a
                target='_blank'
                className='read-more'
                href='https://newsghana.com.gh/nana-a-js-train-of-thought-ep-is-a-fleeting-express-worth-boarding/'
              >
                Ream More
              </a>
            </div>
            <div className='col-md-6 mx-4'>
              <a
                target='_blank'
                href='https://www.ghanaweb.com/GhanaHomePage/entertainment/Nana-A-J-reassures-on-new-single-Bigger-1280410'
              >
                <img
                  className='img-fluid my-5'
                  src='https://newsghana.com.gh/wp-content/uploads/2021/08/Train-of-Thought-696x696.jpg '
                  alt=''
                />
              </a>
            </div>
            <div className='col-md-4 my-auto big'>
              <h3>
                Nana A.J's "Train of Thought" EP is a fleeting express worth
                boarding
              </h3>
              <a
                target='_blank'
                className='read-more'
                href='https://newsghana.com.gh/nana-a-js-train-of-thought-ep-is-a-fleeting-express-worth-boarding/'
              >
                Ream More
              </a>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}

export default News;
